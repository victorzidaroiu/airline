module.exports = (lines, callback) => {
	let debug = require('debug')('airline');
	...
	if (route.totalPassengers <= route.minimumTakeoffLoadPercentage / 100 * aircraft.numberOfSeats)
		route.canFlightProceed = false;
	...
	callback([
		...
		route.cost,
		(passengers.general.length + passengers.loyalty.length + passengers.airline.length) * route.ticketPrice,
		route.revenue,
